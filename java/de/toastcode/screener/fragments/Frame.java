...
*/
    ...
    L_0x00f7:
        throw r5;
        */
        throw new UnsupportedOperationException("Method not decompiled: de.toastcode.screener.fragments.Frame.selectFromDB(java.lang.String):void");
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        this.startDownload.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                Frame.this.startActivity(new Intent(Frame.this.getActivity(), Choose_CompanyActivity.class));
            }
        });
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        this.searchView.setOnQueryTextListener(new OnQueryTextListener() {
            public boolean onQueryTextSubmit(String query) {
                Frame.this.mAdapterGrid.filter(query);
                return true;
            }

            public boolean onQueryTextChange(String newText) {
                Frame.this.mAdapterGrid.filter(newText);
                return true;
            }
        });
    }

    private ArrayList<ArrayList<String>> filter(ArrayList<String> device, ArrayList<String> thumb, String query) {
        int count = 0;
        query = query.toLowerCase();
        ArrayList<ArrayList<String>> mReturn = new ArrayList();
        ArrayList<String> mDevice = new ArrayList();
        ArrayList<String> mThumb = new ArrayList();
        if (query.equals(BuildConfig.FLAVOR)) {
            ...
            while (it.hasNext()) {
                ...
                if (dev.toLowerCase().contains(query)) {
                    mDevice.add(dev);
                    mThumb.add(thumb.get(count));
                }
                count++;
            }
            mReturn.add(mDevice);
